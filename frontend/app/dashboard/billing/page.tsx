  ...
  return (
    <div className="container mx-auto px-4 py-12">
      <header className="mb-12">
        <h1 className="text-xs font-black tracking-[0.3em] text-indigo-500 uppercase mb-2">Checkout</h1>
        <h2 className="text-4xl md:text-5xl font-black text-white tracking-tighter uppercase leading-none">
          Purchase <span className="text-white/20 tracking-normal italic">Credits</span>
        </h2>
      </header>
      ...
      <div className="bg-gradient-to-br from-indigo-600 via-purple-600 to-indigo-900 text-white rounded-[40px] shadow-2xl p-10 mb-16 relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-8 relative z-10">
          <div>
            <div className="text-xs font-black uppercase tracking-[0.2em] opacity-60 mb-2">Available Balance</div>
            <div className="text-6xl font-black tracking-tighter">
              {user?.balance?.toLocaleString() || 0} <span className="text-lg opacity-40 uppercase tracking-widest ml-2">Credits</span>
            </div>
          </div>
          <div className="text-right">
            <div className="text-xs font-black uppercase tracking-[0.2em] opacity-60 mb-2">Current Tier</div>
        ...
        {plans.map((plan) => (
          ...
          >
            ...
            <div className="flex items-baseline gap-1 mb-8">
               ...
               <span className={`text-xs uppercase tracking-widest ${plan.is_popular ? 'text-white/60' : 'text-white/20'}`}>One-time</span>
            </div>
            
            <p className={`text-sm font-medium mb-10 ${plan.is_popular ? 'text-white/80' : 'text-white/40'}`}>
              {plan.credits.toLocaleString()} Prompts for {plan.duration_days} days usage.
            </p>
            ...
            <ul className="space-y-4 mb-12">
              {[
                ...
                plan.is_popular ? 'Priority Support' : 'Standard Support',
                plan.price_eur > 5 ? `Save ${Math.round((1 - (plan.price_eur / plan.credits * 1000)) / 0.03 * 100)}%` : null
            ...
            >
              {purchasing === plan.id ? 'Loading...' : 'Select Plan'}
            </button>
          </div>
        ))}
      </div>
      ...
      <div className="bg-black/40 border border-white/5 rounded-[40px] p-12">
        <h3 className="text-sm font-black text-white uppercase tracking-[0.3em] mb-10 border-b border-white/5 pb-6 text-center">Payment Information</h3>
        <div className="grid md:grid-cols-2 gap-12 text-center md:text-left">
           <div>
              <h4 className="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-4">Secure Checkout</h4>
              <p className="text-xs text-white/30 font-medium leading-relaxed uppercase tracking-widest">
                All transactions are processed through Telegram Tribute API. 
                Encrypted and decentralized for your privacy.
              </p>
           </div>
           <div>
              <h4 className="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-4">Instant Credit</h4>
              <p className="text-xs text-white/30 font-medium leading-relaxed uppercase tracking-widest">
